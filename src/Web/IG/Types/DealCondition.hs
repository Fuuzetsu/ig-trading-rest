...
module Web.IG.Types.DealCondition
  ( DealCondition(..)
  ) where
...
data DealCondition =
    ...
    ACCOUNT_NOT_ENABLED_FOR_TRADING
  ...
  | CANNOT_REMOVE_STOP
    -- | We are not taking opening deals on a Controlled Risk basis on
    -- this market
  | CLOSING_ONLY_TRADES_ACCEPTED_ON_THIS_MARKET
    -- | Resubmitted request does not match the original order.
  | CONFLICTING_ORDER
    -- | Instrument has an error - check the order's currency is the
    -- instrument's currency (see the market's details); otherwise
    -- please contact support.
  | CONTACT_SUPPORT_INSTRUMENT_ERROR
    -- | Sorry we are unable to process this order. The stop or limit
    -- level you have requested is not a valid trading level in the
    -- underlying market.
  | CR_SPACING
    -- | The order has been rejected as it is a duplicate of a
  ...
  | EXCHANGE_MANUAL_OVERRIDE
    ...
    -- Check the sprint market's market details for the allowable
    -- expiries.
  | EXPIRY_LESS_THAN_SPRINT_MARKET_MIN_EXPIRY
    -- | The total size of deals placed on this market in a short
    -- period has exceeded our limits. Please wait before attempting
    -- to open further positions on this market.
  | FINANCE_REPEAT_DEALING
    -- | Ability to force open in different currencies on same market
    -- not allowed
  | FORCE_OPEN_ON_SAME_MARKET_DIFFERENT_CURRENCY
    -- | an error has occurred but no detailed information is
    -- available. Check transaction history or contact support for
    -- further information
  | GENERAL_ERROR
  ...
  | INSTRUMENT_NOT_FOUND
    -- | The account has not enough funds available for the requested
    -- trade
  | INSUFFICIENT_FUNDS
    -- | The market level has moved and has been rejected
  | LEVEL_TOLERANCE_ERROR
    -- | The deal has been rejected because the limit level is
    -- inconsistent with current market price given the direction.
  ...
  | MARKET_CLOSED_WITH_EDITS
    -- | The epic is due to expire shortly, client should deal in the
  ...
  | MARKET_ORDERS_NOT_ALLOWED_ON_INSTRUMNET
  ...
  | MARKET_UNAVAILABLE_TO_CLIENT
    -- | The order size exceeds the instrument's maximum configured
    -- value for auto-hedging the exposure of a deal
  | MAX_AUTO_SIZE_EXCEEDED
    -- | The order size is too small
  | MINIMUM_ORDER_SIZE_ERROR
    -- | The limit level you have requested is closer to the market
    -- level than the existing stop. When the market is closed you can
    -- only move the limit order further away from the current market
    -- level.
  | MOVE_AWAY_ONLY_LIMIT
    -- | The stop level you have requested is closer to the market
    -- level than the existing stop level. When the market is closed
    -- you can only move the stop level further away from the current
    -- market level
  | MOVE_AWAY_ONLY_STOP
    -- | The order level you have requested is moving closer to the
    -- market level than the exisiting order level. When the market is
    -- closed you can only move the order further away from the
    -- current market level.
  | MOVE_AWAY_ONLY_TRIGGER_LEVEL
    -- | Opening CR position in opposite direction to existing CR
    -- position not allowed.
  | OPPOSING_DIRECTION_ORDERS_NOT_ALLOWED
    -- | The deal has been rejected to avoid having long and short
    -- open positions on the same market or having long and short open
    -- positions and working orders on the same epic
  ...
  | ORDER_NOT_FOUND
    -- | The total position size at this stop level is greater than
    -- the size allowed on this market. Please reduce the size of the
    -- order.
  | OVER_NORMAL_MARKET_SIZE
    -- | Position cannot be deleted as it has been partially closed.
  | PARTIALLY_CLOSED_POSITION_NOT_DELETED
    -- | The deal has been rejected because of an existing position.
    -- Either set the 'force open' to be true or cancel opposing
    -- position
  | POSITION_ALREADY_EXISTS_IN_OPPOSITE_DIRECTION
    -- | Position cannot be cancelled. Check transaction history or
    -- contact support for further information.
  | POSITION_NOT_AVAILABLE_TO_CANCEL
    -- | Cannot close this position. Either the position no longer
    -- exists, or the size available to close is less than the size
    -- specified.
  ...
  | REJECT_SPREADBET_ORDER_ON_CFD_ACCOUNT
    -- | Order size is not an increment of the value specified for the
    -- market.
  | SIZE_INCREMENT
    -- | The expiry of the position would have fallen after the
  ...
  | TRAILING_STOP_NOT_ALLOWED
    -- | The operation resulted in an unknown result condition. Check
    -- transaction history or contact support for further information
  | UNKNOWN
    -- | The requested operation has been attempted on the wrong
    -- direction
  | WRONG_SIDE_OF_MARKET
  deriving (Show, Eq, Ord, Generic, Read)

instance Aeson.ToJSON DealCondition where
